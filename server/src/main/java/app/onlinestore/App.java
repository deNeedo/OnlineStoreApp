...
public class App
{
    ...
    public static String client_register(String data) throws Exception
    {
        ...
        {
            stmt = conn.prepareStatement("insert into onlinestore.users (user_id, login, password, first_name, last_name, phone_number) values (?, ?, ?, ?, ?, ?)");
            stmt.setInt(1, 100); stmt.setString(2, data.split(" ")[1]);
